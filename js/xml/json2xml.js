	...
	function convertToXml(json, tagName, parentPath, depth) {
		...
		for (var key in json) {
			if (json.hasOwnProperty(key)) {
				...
				if (settings.ignore.indexOf(propertyPath) == -1) {							//if ($.inArray(propertyPath, settings.ignore) == -1)
					// array, create new child element
					if( typeof(json[key]) === "object" && json[key] instanceof Array){ 		//if ($.isArray(json[key]))
					...
					// primitive value property as attribute
						...
						if(settings.nodes.indexOf(propertyPath) != -1){						//  if ($.inArray(propertyPath, settings.nodes) != -1) 
